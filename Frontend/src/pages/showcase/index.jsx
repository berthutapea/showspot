...
const ShowCase = () => {
  ...
  useEffect(() => {
    ...
    if (location.pathname === '/' && generalShowcaseProjectsData.length > 6) {
      setItems(generalShowcaseProjectsData.slice(0, 6));
    } else {
      setItems(generalShowcaseProjectsData);
    }
  }, [inView, animation, location.pathname, generalShowcaseProjectsData]);

  const filterItem = (category) => {
    const filtered = Items.filter((item) => item.category === category);
    setItems(filtered.slice(0, 6)); // Limit to 6 items after filtering
  };
