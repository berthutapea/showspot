...
import {
  fetchMyProfileStudents,
  updateStudents,
} from '../../../configs/redux/action/myProfileAction';

const ProfileStudentsEdit = () => {
  const dispatch = useDispatch();
  const { id } = useParams();
  const navigate = useNavigate();

  const profileDataUsers =
    useSelector((state) => state.profileDataUsers.profileDataUsers) || {};
  ...
  const handleSubmit = (e) => {
    ...
    dispatch(updateStudents(id, formData, navigate))
      .then(() => {
  ...
  useEffect(() => {
    dispatch(fetchMyProfileStudents(id));
  }, [dispatch, id]);
...
export default ProfileStudentsEdit;
